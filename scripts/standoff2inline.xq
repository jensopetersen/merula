...
declare function local:build-up-annotations($top-level-critical-annotations as element()+, $annotations as element()) as element()* {
    for $annotation at $i in $top-level-critical-annotations
    ...
    let $log := util:log("DEBUG", ("##$annotation-id): ", $annotation-id))
    let $children :=
    ...
    let $children :=
                <children>{(
                    ...
                    where $annotations/annotation[target//id = $child-id]
                    return local:build-up-annotations($children, $annotations))}</children>
    let $children := functx:distinct-deep ($children)
    return 
        local:insert-element($annotation, functx:distinct-deep ($children), $annotation-element-name,  'first-child')
...
let $top-level-critical-annotations := 
    for $top-level-critical-annotation in $top-level-critical-annotations
    return $top-level-critical-annotation
let $log := util:log("DEBUG", ("##$top-level-critical-annotations): ", $top-level-critical-annotations))
return local:build-up-annotations($top-level-critical-annotations, $annotations)