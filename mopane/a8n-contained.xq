...
(:Finds contained a8ns from test a8n:)
...
let $test-a8n := <a8n n="b"><offset>15</offset><range>5</range></a8n>
...
let $results := 
    ...
    return
        ...
        then ()
            ...
            if (
                ($target-offset >= $test-a8n/offset/number() and $target-offset < $test-a8n/offset/number() + $test-a8n/range/number() 
                or
                $target-offset > $test-a8n/offset/number() and $target-offset <= $test-a8n/offset/number() + $test-a8n/range/number() 
                )
            and $target-offset + $target-range < $test-a8n/offset/number() + $test-a8n/range/number())
            then
                $a8n-target
            else ()
